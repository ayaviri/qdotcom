...
import chen_ayaviri.map_representation.CanonicalTileComparator;
...
// Represents an abstract implementation of Dumb And Greedy game playing strategy that computes the next turn action by
// - mapping all tiles to their possible placement positions (sorted) and filtering out those that cannot be placed
// - sorting the remaining candidate tiles and choosing the smallest to create a placement
// If a placement is impossible, attempts to exchange if there are sufficient remaining referee tiles, otherwise pass
public abstract class ADAG implements Strategy {
    // A comparator to sort candidate tiles so that the first element of the sorted list is used for placement first
    ...
    // Produces a pass, exchange, or exhausted sequence of placements given the current state using this strategy
    ...
    // Computes a turn action given the current state and a function dictating how to construct a place action
    ...
    protected TurnAction computeTurnActionHelper(
        ...
        Optional<PlaceAction> placeAction = placeActionConstructor.apply(
                currentState.getMap(),
                currentState.getTiles()
            );
    ...
    protected Optional<PlaceAction> constructOnePlacement(QMap currentMap, List<Tile> currentHand) {
        Optional<Placement> potentialPlacement = this.constructNextPlacement(currentMap, currentHand);
    ...
    public Optional<Placement> constructNextPlacement(QMap currentMap, List<Tile> currentHand) {
        Map<Tile, List<Posn>> filteredHandPositions = this.getValidPositionsForTiles(currentMap, currentHand);
    ...
    // Returns a map from each tile in the given hand to the list of positions on which it can be placed in the given
    // QMap, excluding the tiles with no valid placements
    // Each list of positions is sorted according to this strategy's comparator from most favorable to least
    protected Map<Tile, List<Posn>> getValidPositionsForTiles(QMap currentMap, List<Tile> currentHand) {
    ...
    // This helper is abstracted so that subclasses can implement position sorting. Some strategies use the current
    // state of the map to compare positions while others do not.
    // Modifies the given list of positions.
    abstract protected void sortCandidatePositions(QMap currentMap, List<Posn> candidatePositions);
    ...
    // Returns an optional place action that contains a single placement, if it is present
    private class OnePlacementConstructor implements BiFunction<QMap, List<Tile>, Optional<PlaceAction>> {
    ...
    // Returns an optional place action that contains a sequence of placements, if it is present
    private class PlacementSequenceConstructor implements BiFunction<QMap, List<Tile>, Optional<PlaceAction>> {
        public Optional<PlaceAction> apply(QMap map, List<Tile> hand) {
