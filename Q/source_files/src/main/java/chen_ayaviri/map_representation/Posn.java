...
public class Posn {
    ...
    public List<Posn> getNeighboringRowPositions() {
        return new ArrayList<Posn>(Arrays.asList(
                new TranslateRight().apply(this, 1),
                new TranslateLeft().apply(this, 1)
                ));
    }
    ...
    public List<Posn> getNeighboringColumnPositions() {
        return new ArrayList<Posn>(Arrays.asList(
                new TranslateUp().apply(this, 1),
                new TranslateDown().apply(this, 1)
                ));
    }

    // Given a position, translates this position into a new position relative to that position as if it were the origin
    ...
    public static List<TranslateDirection> getTranslationDirections() {
        return new ArrayList<TranslateDirection>(Arrays.asList(
                new TranslateUp(),
                new TranslateDown(),
                new TranslateRight(),
                new TranslateLeft()
    ...
    // Constructs a new position using this position's Y value and the minimum X value from this and the given position
    ...
    // Returns the absolute value difference between this positions X value and the given position's X value
    public int computeXDistance(Posn other) {
        return Math.abs(other.getX() - this.getX());
    }

    // Returns the absolute value difference between this positions Y value and the given position's Y value
    ...
    public static abstract class TranslateDirection implements BiFunction<Posn, Integer, Posn> {}

    // Represents a function object that translates a given position _n_ units in the -Y (up) direction
    ...
    // Represents a function object that translates a given position _n_ units in the +Y (down) direction
    ...
    // Represents a function object that translates a given position _n_ units in the +X (right) direction
    ...
    // Represents a function object that translates a given position _n_ units in the -X (left) direction
