 ...
 * Down arrow to open save file dialogue. Saves files to a hard-coded filepath. Saving states and
 * selected states fails if Tmp already exists as a file.
 */
public class Observer implements IObserver {
    ...
    public void gameOver() {
        // do nothing. Don't want to close GUI after game is over if we want to calmly view received
        // states
        // for developers
    }
    ...
    protected void renderCurrent(ImageIcon image) {
        image.setImage(
                GameStateRenderer.getAsBufferedImage(states.get(currentStateIndex).visualize()));
    }

    protected void createTempDirectory() {
        // TODO figure out filepath of executable runtime and temp directory hard coded path is
        // declared twice. refactor into a field
    ...
    private void saveReceivedStateAsImage() {
        ...
        try {
            ...
            BufferedImage bufferedImage =
                    GameStateRenderer.getAsBufferedImage(
                            states.get(mostRecentStateIndex).visualize());
