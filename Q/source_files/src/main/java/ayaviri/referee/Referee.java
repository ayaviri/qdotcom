...
import ayaviri.common.TurnResult;
...
// NOTE: If a player misbehaves at any point during the game (including pre-game and post-game
// interactions),
...
public class Referee {
    ...
    // remaining parameters from the given configuration. Injects the given list of player proxies
    // IF
    ...
    protected RoundResult playRound() {
        ...
        for (int index = 0; index < numberOfTurns; index++) {
    ...
    protected Optional<TurnResult> playTurn() {
        ...
        CommunicationResult<TurnAction> turnActionCommunication =
        ...
        if (turnActionCommunication.hasSucceeded()) {
            TurnResult turnResult =
                    this.playTurnWithAction(
                            turnActionCommunication.returnValue().get(), activePlayerState);

            return Optional.of(turnResult);
        } else {
            this.logger.println(
                    String.format(
                            "Could not retrieve %s's turn action, they will be eliminated",
                            activePlayerState.getName()));
    ...
    protected TurnResult playTurnWithAction(TurnAction turnAction, PlayerState activePlayerState) {
        this.logger.println(
                String.format(
                        "%s responded with the following turn action: %s",
                        activePlayerState.getName(), turnAction));
        ...
        if (this.gameState.checkLegalityOf(turnAction)) {
            ...
            this.logger.println(
                    String.format(
                            "An illegal move was played, %s will be eliminated",
                            activePlayerState.getName()));
    ...
    protected TurnResult playLegalTurnWithAction(
            TurnAction turnAction, PlayerState activePlayerState) {
        TurnResult turnResult = this.gameState.performCheckedTurnAction(turnAction);

        if (!this.isTerminalTurn(turnResult)) {
            this.logger.println(
                    String.format(
                            "Turn action was successful and does not end the game by itself, giving %s their new tiles",
                            activePlayerState.getName()));

            // TODO: The specification does not seem to suggest that a player that performs a pass
            // action
    ...
    protected void setupPlayers(List<PlayerState> playerStates) {
        for (PlayerState playerState : playerStates) {
            ActivePlayerInfo activePlayerInfo = this.gameState.getInfoForActivePlayer();
            CommunicationResult<Void> setupResult =
                    this.attemptTimedCommunicationWithReward(
                            new Setup(
                                    playerState.getProxy(),
                                    activePlayerInfo,
                                    activePlayerInfo.getTiles()),
                            playerState.getName(),
                            new PlayerQueueAdvancement(this));
        }
    }
    ...
    // Determines the winning players, communicates to each non-eliminated player whether they won
    // or
    ...
    protected List<String> alertPlayersOfFinalStatus() {
        ...
        for (PlayerState playerState : this.gameState.getPlayerStates()) {
            ...
            CommunicationResult<Void> winCommunication =
                    this.attemptTimedCommunicationWithPunishment(
    ...
    protected boolean isTerminalRound(RoundResult roundResult) {
        return roundResult.hadGameEndingTurn()
                || this.hasNoPlaceActions(roundResult.getTurnActions());
    }
    ...
    // Executes the given callable, which represents a communication attempt with a remote player,
    // with a
    // timeout limit defined by this referee. The failure callback defaults to removal of the player
    // with
    // the given name. Returns the result of the communication
    protected <T> CommunicationResult<T> attemptTimedCommunication(
            Callable<T> callable, String name) {
        TimedCommunication<T> timedCommunication =
                new TimedCommunication.Builder<T>(callable, this.COMMUNICATION_TIMEOUT_SECONDS)
                        .failureCallback(new PlayerRemovalPunishment(this, name))
                        .build();

        return timedCommunication.attempt();
    }

    // Same as above, but executes the given failure callback instead of defaulting to player
    // removal
    protected <T> CommunicationResult<T> attemptTimedCommunicationWithPunishment(
            Callable<T> callable, Supplier<Void> failureCallback) {
        TimedCommunication<T> timedCommunication =
                new TimedCommunication.Builder<T>(callable, this.COMMUNICATION_TIMEOUT_SECONDS)
    ...
    // Same as attemptTimedCommunication, but adds the given success callback, which is executed
    // upon success
    protected <T> CommunicationResult<T> attemptTimedCommunicationWithReward(
            Callable<T> callable, String name, Supplier<Void> successCallback) {
        TimedCommunication<T> timedCommunication =
                new TimedCommunication.Builder<T>(callable, this.COMMUNICATION_TIMEOUT_SECONDS)
                        .failureCallback(new PlayerRemovalPunishment(this, name))
