...
public class ActivePlayerInfo {
    ...
    // Represents the scores of all players in this game in order of turn taking, beginning with the
    // active player
    private final List<Integer> playerScores;

    // The validity of these parameters is not checked here as this object's constructor is
    // protected (only created by the server/game state)
    // TODO: should this constructor be public ? necessary for TileNotOwnedPlayer
    public ActivePlayerInfo(
            QMap map, PlayerState player, int remainingRefereeTiles, List<Integer> playerScores) {
        this.map = map;
    ...
    public static ActivePlayerInfo fromJson(JsonObject jPub) {
        ...
        Iterator<JsonElement> playerScoresIterator =
                jPub.get("players").getAsJsonArray().iterator();
