...
public class QMapImage {
     ...
     * following: - The top left is at (0, 0) - Row 0 contains the topmost tile. - Col 0 contains
     * the leftmost tile. - The bottom right is at (N - 1, N - 1) - Row N - 1 contains the
     * bottom-most tile. - Col N - 1 contains the rightmost tile.
     *
     * @param padded
     * @return a grid representation of the map,
     */
    public List<List<Optional<Tile>>> getGrid(int padded) {
        List<List<Optional<Tile>>> grid =
                initializeGrid(bounds.numRows() + 2 * padded, bounds.numCols() + 2 * padded);
