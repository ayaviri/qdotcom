...
public class GameStateRenderer {
    ...
    public static void render(GameStateImage gameStateImage) {
        ...
        renderComponent(contents, mergeImagePlayerStatesTilesLeft(gameStateImage), 0, 0, 600, 250);
        contents.add(Box.createRigidArea(new Dimension(0, 50)));
        renderComponent(
                contents, QMapRenderer.generateImage(gameStateImage.qMap(), 1), 50, 0, 600, 600);
    ...
    private static void renderComponent(
            JPanel panel, BufferedImage component, int x, int y, int maxWidth, int maxHeight) {
        double scale =
                Math.min(
                        1.0,
                        Math.min(
                                (double) maxWidth / component.getWidth(),
    ...
    private static BufferedImage mergeImagePlayerStatesTilesLeft(GameStateImage gameStateImage) {
        return ImageUtil.joinHorizontal(
                PlayersRenderer.generateImage(gameStateImage.playerStates()),
