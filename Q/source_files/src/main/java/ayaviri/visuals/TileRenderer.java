...
import ayaviri.map_representation.shapes.Circle;
import ayaviri.map_representation.shapes.Clover;
import ayaviri.map_representation.shapes.Diamond;
import ayaviri.map_representation.shapes.EightStar;
...
public class TileRenderer {
    ...
    // TODO separate methods for render tile, render hidden tile (censored gs), render empty
    // space...
    ...
    public static BufferedImage renderRow(
            List<Optional<Tile>> tiles, int size, int spaceBetween, boolean drawBorder) {
        return tiles.stream()
                .map(tile -> TileRenderer.render(tile, size, drawBorder))
                .reduce(
                        new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB),
                        (accumulator, tile) ->
                                ImageUtil.joinHorizontal(accumulator, tile, spaceBetween));
    }
    ...
    public static BufferedImage drawTile(Optional<Tile> optionalTile, boolean drawBorder) {
        ...
        if (optionalTile.isPresent()) {
            ...
            //            break;
            //          case EIGHT_STAR:
            //            int[] x1 = {16, 40, 50, 60, 84, 70, 100, 70, 84, 60, 50, 40, 16, 30, 0,
            // 30};
            //            int[] y1 = {16, 30, 0, 30, 16, 40, 50, 60, 84, 70, 100, 70, 84, 60, 50,
            // 40};
            //            gtd.fill(new Polygon(x1, y1, 16));
            //            break;
            //          case CLOVER:
