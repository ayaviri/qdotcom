...
public class PlayersRenderer {
    ...
    public static BufferedImage generateImage(PlayersImage players) {
        return ImageUtil.joinHorizontal(
                drawCurrentPlayerArrow(players.current()),
                ImageUtil.joinHorizontal(
                        drawNames(players),
                        ImageUtil.joinHorizontal(drawScores(players), drawTiles(players))));
    }

    private static BufferedImage drawStringImage(
            String text, Float fontsize, int width, int height) {
