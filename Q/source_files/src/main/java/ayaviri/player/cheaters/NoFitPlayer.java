...
// Represents a player that requests a placement of a tile that does not match its neighbors in
// response
...
public class NoFitPlayer extends ACheatingPlayer {
    ...
    protected TurnAction thenCheat(ActivePlayerInfo currentState) {
        return new GameState.PlaceAction(this.getNonMatchingPlacement(currentState).get());
    }
    ...
    protected Optional<Placement> getNonMatchingPlacement(ActivePlayerInfo currentState) {
        ...
        if (turnAction instanceof GameState.PlaceAction) {
            // NOTE: relies on the invariant held by PlaceAction that the sequence of placements is
            // nonempty
            Placement placement = ((GameState.PlaceAction) turnAction).getPlacements().get(0);
            List<Tile> neighboringTiles =
                    stateCopy.getMap().getNeighboringTiles(placement.getPosition());
            Set<Tile> nonMatchingTiles = Tiles.getNonMatchingTiles(neighboringTiles);
            Optional<Tile> chosenNonMatchingTile =
                    this.selectFromHand(stateCopy.getTiles(), nonMatchingTiles);
