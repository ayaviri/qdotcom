...
public class AStar extends AShape {
    ...
    // the angle between the x-axis and the first vertex above and to the right of the x-axis in
    // radians
    ...
    public Shape draw(double topLeftX, double topLeftY) {
        ...
        for (int index = 0; index < this.numberOfRays * 2; index++) {
            ...
            if (index == 0) {
                path.moveTo(
                        topLeftX + this.CENTER_X + relativeX, topLeftY + this.CENTER_Y + relativeY);
            } else {
                path.lineTo(
                        topLeftX + this.CENTER_X + relativeX, topLeftY + this.CENTER_Y + relativeY);
            }
