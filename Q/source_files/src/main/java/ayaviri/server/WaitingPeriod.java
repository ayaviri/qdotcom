...
import ayaviri.player.PlayerProxy;
...
public class WaitingPeriod implements Callable<Void> {
    ...
    protected Optional<IPlayer> getNewPlayerProxy() {
        try {
            ...
            CommunicationResult<String> nameCommunicationResult =
                    this.attemptToGetPlayerName(playerSocket);
    ...
    // Attempts to the get a player's name through the given socket using this WaitingPeriod's
    // timeout
    protected CommunicationResult<String> attemptToGetPlayerName(Socket playerSocket) {
        TimedCommunication<String> timedCommunication =
    ...
    // and the socket for communcation with the remote player. Returns empty if the instantiation
    // fails
    // NOTE: Assumes that the CommunicationResult has succeeded
    protected Optional<IPlayer> constructNamedPlayerProxy(
            CommunicationResult<String> nameCommunicationResult, Socket playerSocket) {
