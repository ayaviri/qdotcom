...
public class Server {
    ...
    public GameResult runGameToCompletion() {
        ...
        // TODO: This constant ties the server and referee components... Can we avoid doing so ? If
        // so, how ?
    ...
    protected List<IPlayer> waitForSignUps() {
        ...
        } while (this.canSignUpMorePlayers(players.size())
                && this.canWaitAnotherRound(waitingRoundsCompleted));
    ...
    protected void closeAllOpenPlayerSockets() {
        this.logger.println("Closing all open sockets");

        for (Closeable socket : this.openedSockets) {
            try {
                socket.close();
            } catch (IOException e) {
                this.logger.println("There was a failure during the closure of a socket");
                // TODO: This should probably just be logged, the OS will close them for us anyway
            }
        }
    }

    // Creates a new server socket with the given port number and initialises this server's socket
    // to it
